...
5. [이미지화 가능]: 원리를 직관적이고 직설적으로 묘사하는 실제 과학 사진이나 자연 현상 사진으로 시각화하기 쉬운 주제여야 합니다. 무료 이미지 플랫폼(Pixabay)에서 고품질 사진이 검색될 수 있도록 1~3단어 수준의 명확하고 물리적인 영문 키워드(예: 'lactobacillus', 'nebula', 'microscope cells', 'solar eclipse')를 뽑을 수 있을 것.
   - ⚠️ **[중요 - 추상화/비유 금지]**: '암세포 파괴 시뮬레이션'이나 '양자 컴퓨터 톱니바퀴 모식도'처럼 복잡한 시각적 CG나 추상적 모식도를 유도하는 키워드 대신, 물리적 실체나 자연 현상(예: 'cancer cell', 'computer chip', 'laser beam', 'dna double helix')을 포토그래피(Photography) 스타일로 직설적으로 묘사하는 단순한 키워드만 선정해야 합니다. 추상적인 비유(톱니바퀴, AI 사람, 그래픽 합성 등)를 유도하지 마십시오.
   - 키워드는 절대 복잡하게 꾸미지 말고, 검색 결과가 풍부하게 나오는 실제 존재하는 사물이나 자연 현상 위주로 선정하십시오.
